...
A real-time admin console with sidebar navigation and six widgets. A **live event feed** shows every viewer interaction as a terminal-style stream — newest-first, monospace, color-coded by event type. A **sessions list** tracks every viewer with live/idle/disconnected status indicators; click to expand the full event timeline inline. A **films widget** shows per-film metadata, aggregate stats, and the full cue list with timestamps. A **system health** panel shows server uptime, WebSocket connections, session/event totals, and TTS cache stats. An **AI usage** panel shows message counts, average response length, and recent conversations. A **timeline heatmap** renders an SVG stacked bar chart showing event density across the film's timeline, with cue markers and hover tooltips. Everything updates over WebSocket — no polling. Stats increment live as viewers interact; the snapshot-on-connect corrects any drift on reconnect.
...
2. **Production video serving with self-contained film packages.** Videos are served by the Go backend via `http.ServeContent` — not from a static directory — which handles `Range` headers (HTTP 206), `If-Modified-Since`, and correct `Content-Type` automatically. This is the production serving path where you'd add auth, access logging, and CDN origin behavior. Each film is a self-contained directory (`data/films/heat/`) with the video, cue definitions, and metadata together — adding a new film is "create a folder, drop files in it" with no code changes or schema migrations. Using a full-length ~2GB movie instead of a 30-second clip exercises realistic conditions: range requests across a large file, seeking behavior, and real I/O patterns.
...
4. **Live analytics dashboard that feels like a real product.** The admin view goes well beyond "who's watching, what they're doing." Per-film aggregate stats (sessions, plays, chat messages, cues triggered) are included in the WebSocket snapshot on connect, then locally incremented on the frontend as events arrive — the dashboard feels live without extra bandwidth, and any drift self-corrects on reconnect. The timeline heatmap renders an inline SVG stacked bar chart (no charting library) showing event density across the film's runtime with cue markers and hover tooltips. System health, AI usage analytics, and recent conversations round out six dashboard widgets — all reactive over a single WebSocket connection.

5. **AI as a first-class part of the viewing experience, not a bolted-on feature.** The Go backend proxies all AI traffic as a stateless SSE relay — the frontend owns conversation history, the API key never touches the browser, and every exchange is persisted as events for analytics. When the Sunset API key had issues, I built switchable providers (Sunset proxy vs direct Anthropic for chat, Sunset TTS vs browser Web Speech API) rather than stubbing — flip an env var and the production path lights up. But the real differentiator is how AI integrates into the experience: chat responses are spoken aloud via TTS after streaming completes, and voice cue narrations appear in the chat log with per-bubble play/pause controls. The AI doesn't just answer questions — it narrates the film and maintains a shared conversation thread between triggered cues and user-initiated chat.
